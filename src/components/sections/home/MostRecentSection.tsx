...
const MostRecentSection = ({mostRecentArticles, allMostRecentGridArticles, popularArticles}:MostRecentSectionProps) => {
    ...
    const handlePageChandge = (page: number) => {
        ...
        <div className='px-4 sm:px-6 lg:px-8'>
            <div className='flex flex-col lg:flex-row lg:space-x-8'>
                ...
                <div className='w-full lg:w-8/12'>
                    ...
                    <div>
                        {mostRecentArticles.map((article) => <ArticleCardMostRecent key={article._id} article={article}/>)}
                    </div>
                    {/* divider line */}
                    <div className='border-t border-gray-300 my-8'></div>
                    {/* 2x2 all most recent grid articles */}
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
                        {currentGridArticles.map((article) => <MostRecentGridArticles key={article._id} article={article}/>)}
                    </div>
                    {/* pagination */}
                    <Pagination totalPages={totalPages} currentPage={currentPage} onPageChange={handlePageChandge}/>
                </div>
                {/* right side content */}
                <div className='w-full lg:w-4/12 lg:pl-8'>
                    <div className='sticky top-24 lg:h-fit lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto'>
                        ...
                        <div className='list-none p-0'>
                            {popularArticles.map((article, index) => <TrendingArticleItem key={index} article={article} index={index}/>)}
                        </div>
                    </div>
                </div>
            </div>
